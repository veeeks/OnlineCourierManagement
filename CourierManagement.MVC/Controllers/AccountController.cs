...
using CourierManagement.MVC.Models;
...
namespace CourierManagement.MVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                httpClient.BaseAddress = new Uri("http://localhost:52210/");
                ...
                var result = await httpClient.PostAsync("token", content);
                if(result.StatusCode==System.Net.HttpStatusCode.BadRequest)
                {
                    ModelState.AddModelError(string.Empty, "Invalid emailId or password");
                    return View();
                }
            ...
            var res = await client.GetAsync($"DeliveryExecutives/GetExecutiveEmail/{email}/");

            var DEmail = await res.Content.ReadAsAsync<string>();
            if (DEmail != null)
            {
                Session["CurrentEmail"] = email;
                return RedirectToAction("Index", "DeliveryExecutives");
            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (Session["DE"] != null)
                    {
                        string de = Session["DE"].ToString();
                        Session["DEmail"] = model.Email;
                        Session["DEPassword"] = model.Password;
                        return RedirectToAction("AddNewDe", "Administrators");
                    }
